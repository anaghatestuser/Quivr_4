...
if __name__ == "__main__":
    ...
    # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:40703df3628713ad01f67b2b011c26027306f6a42b9dffcd238ce036debc6c5c'
    ...
    while True:
        ...
        if question.lower() == "exit":
            ...
            # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:2f9f24a5009e3a821499e2a7cdf35a9fd2e0bbc8fee15e678b120180d46c2896'
            _gr_client = _lineaje_load_gr_client()
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:a98c4663ef2ddb841a1c6e4da18031773dd2c9c6db4592be0a95dedcfb4610d6'
        _gr_client = _lineaje_load_gr_client()
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:333e8f0b353e1724ba24cce5785ed44625d5f654750b667c7596249411363f76'
        _gr_client = _lineaje_load_gr_client()
