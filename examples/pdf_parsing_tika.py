...
if __name__ == "__main__":
    ...
    # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:a688ff436888322ffc60e2a431014e6f572e454c91c14900d1530bd0e4dfbc41'
    ...
    while True:
        ...
        if question.lower() == "exit":
            ...
            # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:815a59272a5d6f688bb02d062685ac99c674c4093e4ddbac07e71257b7e19f9b'
            _gr_client = _lineaje_load_gr_client()
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:f0aee67dc3d7ce2c1f9dcca686c8399c5e5a0980494e77df6aad2888f1ae8237'
        _gr_client = _lineaje_load_gr_client()
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_DAT_SEC_023 (Redact PII from uploaded files.); AI_DAT_SEC_024 (Uploaded files must not contain PII (Singapore).). Mask/block; do not remove without review. site_id='site:sha256:bdc33dffb183206e5285cd37849edd41cbd9aa4a6f272fb9511b02560a4a39d1'
        _gr_client = _lineaje_load_gr_client()
